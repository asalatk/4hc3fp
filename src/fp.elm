...
title : String
title = "Word Count"
occs:String -> List String->Int
occs a b = List.length (List.filter (\x -> x ==a)  b)
iconWidth = 128
iconHeight = 64
yScale y = y
...
createGraphY2 w s maxCount y =
    group [filled (rgb 135 206 250) (GraphicSVG.rect w 1) |> move ( 0, y * s ), filled (rgb 135 206 250) (GraphicSVG.rect 0.75 w) |> move ( -64, y * s ), 
   GraphicSVG.text (String.fromFloat ((y+maxCount)*0.5)) |> GraphicSVG.size 2.5 |> filled black |> move ( -63, y * s )]
   
graphPaper : Float -> Float -> Shape notification
graphPaper s maxCount =
    group ( List.map (createGraphY2 3000 s maxCount) (List.map toFloat (List.range (round (-800/s)) (round (800/s)))))
...
barChart wordCounts time =
  ...
  in -- put scale here 
     ...
     [graphPaper (scaleHeight*0.5) (toFloat maxCount)] 
      ++ (List.indexedMap bar (List.drop (List.length(wordCounts)-20)   (sortByFirst wordCounts)))
            |> group
...
update msg model =
    case msg of
        ...
        NewText txt -> ( {model | wordCounts = List.indexedMap ( \ idx wrd -> (occs wrd (String.words(txt)),wrd) )
                                                 <|  List.Extra.unique (String.words txt)
